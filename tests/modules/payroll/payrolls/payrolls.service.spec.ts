...
describe('PayrollsService', () => {
    ...
    describe('createDraftAsync', () => {
        ...
        it('should create draft payroll with correct calculations', async () => {
            ...
            prismaClient.employee.findUnique.mockResolvedValue(mockEmployee);
            prismaClient.currency.findUnique.mockResolvedValue(mockCurrency);
            prismaClient.taxBracket.findFirst.mockResolvedValue(mockTaxBracket);
            prismaClient.$transaction.mockImplementation(async (callback) => {
                return callback({
                    payroll: {
                        create: jest.fn().mockResolvedValue(createdPayroll),
                    },
                    payrollItems: { createMany: jest.fn() },
                    taxCalculation: { create: jest.fn() },
                });
            });
            prismaClient.payroll.findUnique.mockResolvedValue({
                ...createdPayroll,
                items: [],
                taxCalculation: null,
            });
            ...
            expect(prismaClient.employee.findUnique).toHaveBeenCalledWith({
        ...
        it('should fail if employee not found', async () => {
            prismaClient.employee.findUnique.mockResolvedValue(null);
        ...
        it('should fail if currency is invalid', async () => {
            prismaClient.employee.findUnique.mockResolvedValue(mockEmployee);
            prismaClient.currency.findUnique.mockResolvedValue(null);
        ...
        it('should set tax to 0 for tax exempt employee', async () => {
            ...
            prismaClient.employee.findUnique.mockResolvedValue(exemptEmployee);
            prismaClient.currency.findUnique.mockResolvedValue(mockCurrency);
            ...
            expect(prismaClient.taxBracket.findFirst).not.toHaveBeenCalled();
        });
        ...
        it('should use basicSalaryOverride when provided', async () => {
            ...
            const result = await service.createDraftAsync(
                dtoWithOverride,
                'user-1',
            );
    ...
    describe('finalizeAsync', () => {
        it('should finalize pending payroll', async () => {
            ...
            prismaClient.payroll.findUnique.mockResolvedValueOnce(
                pendingPayroll,
            );
            prismaClient.payroll.update.mockResolvedValue({
                ...pendingPayroll,
            ...
            expect(result.isSuccess).toBe(true);
            expect(prismaClient.payroll.update).toHaveBeenCalledWith({
                where: { id: 'payroll-1' },
        ...
        it('should fail if payroll not found', async () => {
            prismaClient.payroll.findUnique.mockResolvedValue(null);

            const result = await service.finalizeAsync(
                'non-existent',
                'user-1',
            );
    ...
    describe('findByIdAsync', () => {
        beforeEach(() => {
            ...
            mockPrismaClient.payroll.findUnique.mockReset();
        });
        ...
        it('should return payroll with items and tax', async () => {
            ...
            mockPrismaClient.payroll.findUnique.mockResolvedValue(payroll);

            const result = await service.findByIdAsync('payroll-1');

            expect(result.isSuccess).toBe(true);
            expect(result.getData()).toBeDefined();
            expect(mockPrismaClient.payroll.findUnique).toHaveBeenCalled();
        });

        it('should fail if payroll not found', async () => {
            mockPrismaClient.payroll.findUnique.mockResolvedValue(null);
        ...
        it('should handle database errors', async () => {
            mockPrismaClient.payroll.findUnique.mockRejectedValue(
    ...
    describe('findAllAsync', () => {
        it('should return all payrolls', async () => {
            ...
            prismaClient.payroll.findMany.mockResolvedValue(payrolls);
        ...
        it('should filter by employeeId', async () => {
            prismaClient.payroll.findMany.mockResolvedValue([]);

            await service.findAllAsync({ employeeId: 'emp-1' });

            expect(prismaClient.payroll.findMany).toHaveBeenCalledWith(
